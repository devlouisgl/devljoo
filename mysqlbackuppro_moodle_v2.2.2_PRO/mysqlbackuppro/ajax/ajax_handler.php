...
try {
    switch ($action) {
        ...
        case 's3_list_objects':
            ...
            $files = $result['files'];
            if ($filter_ext !== '') {
                $files = array_values(array_filter($files, function($f) use ($filter_ext) {
                    return local_mysqlbackuppro_ends_with(strtolower($f['name']), strtolower($filter_ext));
                }));
            }
        ...
        case 'test_s3':
            ...
            $effective_access = $access !== '' ? $access : crypto::decrypt((string) $old_access);
            $effective_secret = $secret !== '' ? $secret : crypto::decrypt((string) $old_secret);
            if ($effective_access === '' || $effective_secret === '') {
                $response = ['success' => false, 'message' => 'Access key and secret key are required for the S3 connection test.'];
                break;
            }
            ...
            set_config('s3_access_key', crypto::encrypt($effective_access), 'local_mysqlbackuppro');
            set_config('s3_secret_key', crypto::encrypt($effective_secret), 'local_mysqlbackuppro');
        ...
        case 'save_settings':
            ...
            foreach ($text_fields as $field => $paramtype) {
                ...
                if ($field === 's3_endpoint' && $value !== '' && !preg_match('#^https?://#i', $value)) {
                    $value = 'https://' . $value;
                }
            ...
            if (!empty($email) && !validate_email($email)) {
                $response = ['success' => false, 'message' => 'Invalid notification email address.'];
                break;
            }
