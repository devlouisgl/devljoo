...
class s3native {
    ...
    public function __construct() {
        $this->endpoint   = rtrim((string) local_mysqlbackuppro_get_config('s3_endpoint', ''), '/');
        if ($this->endpoint !== '' && !preg_match('#^https?://#i', $this->endpoint)) {
            $this->endpoint = 'https://' . $this->endpoint;
        }
        $this->region     = (string) local_mysqlbackuppro_get_config('s3_region', 'default');
        $this->bucket     = (string) local_mysqlbackuppro_get_config('s3_bucket', '');
        $this->access_key = crypto::decrypt((string) local_mysqlbackuppro_get_config('s3_access_key', '')); 
        $this->secret_key = crypto::decrypt((string) local_mysqlbackuppro_get_config('s3_secret_key', '')); 
        $this->path_style = ((string) local_mysqlbackuppro_get_config('s3_path_style', '1')) === '1';
    }
    ...
    public static function configured(): bool {
        $bucket   = (string) local_mysqlbackuppro_get_config('s3_bucket', '');
        $access   = crypto::decrypt((string) local_mysqlbackuppro_get_config('s3_access_key', ''));
        $secret   = crypto::decrypt((string) local_mysqlbackuppro_get_config('s3_secret_key', ''));
        $endpoint = (string) local_mysqlbackuppro_get_config('s3_endpoint', '');
        return trim($bucket) !== '' && trim($access) !== '' && trim($secret) !== '' && trim($endpoint) !== '';
    }
    ...
    private function request(string $method, string $url, array $opts = []): array {
        ...
        if (!function_exists('curl_init')) {
            return ['success' => false, 'code' => 0, 'error' => 'PHP cURL extension is not installed or enabled.', 'body' => ''];
        }
    ...
    public function list_buckets(): array {
        if ($this->endpoint === '' || $this->bucket === '' || $this->access_key === '' || $this->secret_key === '') {
            return ['success' => false, 'message' => 'S3 configuration is incomplete. Endpoint, bucket, access key and secret key are required.'];
        }

        // Most S3-compatible providers allow bucket-level ListObjectsV2 but deny ListAllMyBuckets.
        $url = $this->base_url('') . '?list-type=2&max-keys=1';
        $res = $this->request('GET', $url);
        if ($res['success']) {
            logger::info('S3 connection successful! Bucket accessible: ' . $this->bucket);
            return [
                'success' => true,
    ...
    public function search_objects(
        ...
        do {
            ...
            foreach ($result['files'] as $file) {
                if ($filter_ext !== '' && substr(strtolower($file['name']), -strlen(strtolower($filter_ext))) !== strtolower($filter_ext)) {
                    continue;
                }
