...
try {
    switch ($action) {
        ...
        case 'save_settings':
            ...
            set_config('enabled', $enabled ? '1' : '0', 'local_mysqlbackuppro');

            $compress = optional_param('compress_backup', '0', PARAM_RAW_TRIMMED);
            set_config('compress_backup', $compress ? '1' : '0', 'local_mysqlbackuppro');

            $s3_path_style = optional_param('s3_path_style', '0', PARAM_RAW_TRIMMED);
            set_config('s3_path_style', $s3_path_style ? '1' : '0', 'local_mysqlbackuppro');
