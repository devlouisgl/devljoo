...
class backup {
    ...
    private function dump_sql(string $filepath): array {
        ...
        $version = get_config('local_mysqlbackuppro', 'version') ?: '2.2.0';
        fwrite($fp, "-- MySQL Backup Pro v{$version}\n");
        ...
        foreach ($rows as $r) {
            ...
            $cols = $this->db->get_records_sql("SHOW COLUMNS FROM `{$tbl}`");
            ...
            for ($off = 0; $off < $cnt; $off += $batch) {
                ...
                foreach ($data as $row) {
                    ...
                    foreach ((array)$row as $val) {
                        $v[] = ($val === null) ? 'NULL' : "'" . $this->db->sql_compare_text_special((string)$val) . "'";
                    }
                    $vals[] = '(' . implode(',', $v) . ')';
                    $total_rows++;
                }
